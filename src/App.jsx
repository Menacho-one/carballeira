...
function PhotoStrip({ photos, onAdd, onDel }) {
  const ref = useRef();
  return (
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        {photos.map((p, i) => (
          <div key={i} style={{ position:"relative" }}>
            <img src={p} alt="" style={{ width:72, height:72, objectFit:"cover", borderRadius:6, border:`1px solid ${C.soil}` }} />
...
export default function App() {
  ...
  const [trees, setTrees]     = useState(() => load("cb_trees", INIT_TREES));
  const [species, setSpecies] = useState(() => load("cb_species", INIT_SPECIES));
  const [visits, setVisits]   = useState(() => load("cb_visits", []));
  const [fauna, setFauna]     = useState(() => load("cb_fauna", [
  ...
  // persist
  useEffect(() => save("cb_trees", trees),   [trees]);
  useEffect(() => save("cb_species", species),[species]);
  useEffect(() => save("cb_visits", visits),  [visits]);
  useEffect(() => save("cb_fauna", fauna),    [fauna]);
  ...
  function addVisit() {
    ...
    const next = [v, ...visits];
    setVisits(next); setNewVisit(false); setNvData({ date:"", weather:"", notes:"", photos:[] }); setSelVisit(v);
  }
  function delVisit(id) { setVisits(v=>v.filter(x=>x.id!==id)); setSelVisit(null); }

  function addFaunaEntry() {
    const f = { id: Date.now(), ...nfData };
    setFauna(prev=>[f,...prev]); setNewFauna(false); setNfData({ name:"", emoji:"🦁", date:"", notes:"", photos:[] });
  }
  ...
  return (
    <div style={{
        ...
        {tab==="Visitas" && <>
          ...
          {newVisit && (
            <div style={{ background:"#1e2a14", border:`1px solid ${C.gold}`, borderRadius:10, padding:16, marginBottom:14 }}>
              ...
              <PhotoStrip photos={nvData.photos} onAdd={p=>setNvData({...nvData,photos:[...nvData.photos,p]})} onDel={i=>setNvData({...nvData,photos:nvData.photos.filter((_,j)=>j!==i)})} />
                  ...
                  <div style={{ background:C.ink, border:`1px solid ${C.gold}44`, borderRadius:"0 0 10px 10px", padding:14, marginTop:-4 }}>
                    ...
                    {v.photos?.length>0 && (
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:10 }}>
                        {v.photos.map((p,i)=>(
                          <img key={i} src={p} alt="" style={{ height:90, borderRadius:6, border:`1px solid ${C.soil}`, objectFit:"cover" }} />
                        ))}
                      </div>
                    )}
      ...
      <div style={{ textAlign:"center", padding:"20px", color:"#3a4a2a", fontSize:11 }}>
        Cuaderno de Campo · A Carballeira Familiar · Todo gardado localmente 💾
      </div>
    </div>
  );
}
